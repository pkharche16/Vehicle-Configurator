...
const AlternateModifier = () => {
    ...
    const fetchComponents = async () => {
      try {
        const response = await fetch(
          `http://localhost:8081/api/vehicles/config/${selectedId}/Y`
        );
        if (!response.ok) {
  ...
  const handleCheckboxChange = async (compId, checked) => {
    if (checked) {
      ...
      try {
        const response = await fetch(
          `http://localhost:8081/api/alternate-components/${selectedId}/${compId}`
        );
        if (!response.ok) {
  ...
  //           const response = await fetch(
  //             `http://localhost:8081/api/alternate-components/alt/${selectedId}/${id}`
  //           );
  //           if (!response.ok) {
